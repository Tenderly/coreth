...
func accessListGas(rules params.Rules, accessList types.AccessList) (uint64, error) {
	...
	for _, accessTuple := range accessList {
		...
		if !ok {
			...
			predicateGas, err := predicaterContract.PredicateGas(accessTuple.StorageKeys, rules)
			if err != nil {
				return 0, err
			}
...
func TransactionToMessage(tx *types.Transaction, s types.Signer, baseFee *big.Int) (*Message, error) {
	...
	if baseFee != nil {
		msg.GasPrice = math2.BigMin(msg.GasPrice.Add(msg.GasTipCap, baseFee), msg.GasFeeCap)
	}
	var err error
